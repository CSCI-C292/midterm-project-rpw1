...
export(String, FILE, "*.tscn") var _level_select_path
...
# variables is pointing to. In this case it starts the game, goes to level select or quits.
func ui_accept() -> void:
	if self._selector == 0:
		get_tree().change_scene(self._start_game_path)
	elif self._selector == 1:
		get_tree().change_scene(self._level_select_path)
